...
def parse_config(path: Path) -> DeployConfig:
    ...
    config = DeployConfig(username, port, password, sudo_password, deploy_home, collector_addr, grpc_batch_bytes,
                          grpc_timeout_ms, observer_path, offset, output_file, nodes)
...
def render_start_script(node: Node) -> str:
...
{incomplete}exec "$DEPLOY_HOME/bin/uprobe" "{observer}" "{offset}" "$DEPLOY_HOME/{node.output_file}" "$DEPLOY_HOME/conf/uprobe.conf"
"""


def make_package(config: DeployConfig, node: Node, binary: Path, work_dir: Path) -> Path:
    package_root = work_dir / f"agent-{node.name}"
    if package_root.exists():
        shutil.rmtree(package_root)
    for subdir in ("bin", "conf", "lib", "run", "logs"):
        (package_root / subdir).mkdir(parents=True)

    shutil.copy2(binary, package_root / "bin" / "uprobe")
    write_text(package_root / "conf" / "uprobe.conf", render_uprobe_conf(config, node))
    write_text(package_root / "run" / "start_agent.sh", render_start_script(node), 0o755)
...
def deploy_node(config: DeployConfig, node: Node, archive: Path, dry_run: bool) -> NodeResult:
    ...
    require_cmd(ssh_base(config, node.ip) + [f"mkdir -p {quote_arg(node.deploy_home)}"], dry_run=dry_run)
...
def status_node(config: DeployConfig, node: Node, dry_run: bool) -> NodeResult:
    collector_host, collector_port = split_collector(config.collector_addr)
    tcp_check = "collector_tcp=not_configured"
    if collector_host and collector_port:
        tcp_check = (
            f"timeout 3 bash -c '</dev/tcp/{collector_host}/{collector_port}' >/dev/null 2>&1 "
            f"&& echo collector_tcp=ok || echo collector_tcp=fail"
        )
    pattern = node.deploy_home + "/bin/[u]probe"
    cmd = (
        ...
        f"pids=$(pgrep -f {quote_arg(pattern)} || true); "
        f"if [ -n \"$pids\" ]; then running=yes; else running=no; fi; "
        f"if grep -q 'uprobe attach success' {quote_arg(node.deploy_home)}/logs/uprobe.log 2>/dev/null; then attach=ok; "
        f"elif grep -q 'Failed to attach\|Failed to open\|Invalid offset' {quote_arg(node.deploy_home)}/logs/uprobe.log 2>/dev/null; then attach=fail; "
        f"else attach=unknown; fi; "
        f"grpc=unknown; "
...
def prepare_archives(args: argparse.Namespace, config: DeployConfig, build_dir: Path) -> Dict[str, Path]:
    ...
    for node in config.nodes:
        archives[node.name] = make_package(config, node, binary, build_dir)
        print(f"PACKAGE {node.name} {node.ip}: {archives[node.name]}")
    return archives
